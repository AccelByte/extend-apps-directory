  ...
  <Card classNames={{ body: styles.repositoryCardBody }}>
    <div>
      ...
      <div className={styles.repositoryDetails}>
        {type ? (
          <Tag className={styles.repositoryMeta}>
            {mapRepositoryTypeToText[type]}
          </Tag>
        ) : (
          <Tag>Primary</Tag>
        )}
        •
        <span>
          <Badge dot color={mapLanguageToColor[language as Language]} />{" "}
          {language}
        </span>
        {appRecommendedResource && (
          <>
            • <div>Memory: {appRecommendedResource.memory}</div>•{" "}
            <div>CPU: {appRecommendedResource.cpu}</div>
          </>
        )}
      </div>
    </div>
...
export function ExtendDirectoryDetailModal() {
  ...
  const extendApp = extendApps.find(
    (app) => app.id === appId
  ) as ExtendDirectoryAppInfo;

  if (!extendApp) return null;

  const isComingSoon =
    !extendApp.repositories || extendApp.repositories.length === 0;
  ...
  return (
    ...
    >
      ...
      <div
        className={`d-flex align-items-center justify-content-between ${styles.modalHeader}`}
      >
        <div className="d-flex align-items-center" style={{ gap: 12 }}>
          <img
            src={extendApp.imageUrl}
            width={64}
            height={64}
      ...
      <div className={styles.modalContent}>
        <div>
          ...
          <Paragraph style={{ color: "#000000A6" }}>
            {extendApp.description}
          </Paragraph>
        </div>
        {!isComingSoon && (
          <div>
